...
function JoinScreen({ name, roomCode, setName, setRoomCode, join }) {
  return (
    <main className="joinShell">
      <div className="joinGlow joinGlowLeft" />
      <div className="joinGlow joinGlowRight" />
      <section className="joinCard">
        <div className="joinHeader">
          <BrandMark />
          <div className="joinStatus">live multiplayer</div>
        </div>

        <div className="joinHero">
          <p className="kicker">An illustrated search and solve escape room</p>
          <h1>The Midnight Curio</h1>
          <p className="joinLead">
            Explore the same strange apartment together, collect shared clues, and solve the locks before the apartment decides it likes you too much.
          </p>
        </div>
...
function GameExperience({ playerName, roomCode, leave }) {
  ...
  const [blacklightOn, setBlacklightOn] = useState(false);
  ...
  const toggleBlacklight = () => {
    setBlacklightOn((current) => {
      const next = !current;
      if (next) {
        setRevealBurst(true);
        setTimeout(() => setRevealBurst(false), 1500);
      }
      return next;
    });
  };
    ...
    return (
      <main className="victoryShell">
        ...
        <section className="victoryCard">
          ...
          <h1>You escaped.</h1>
          <p>
            The Odd Door finally swings open. Behind it: a perfectly ordinary hallway, which somehow feels even stranger.
            Take the win, take the screenshot, and go enjoy the night.
          </p>
    ...
    <main className="gameShell">
      <aside className="sidePanel leftPanel">
        <div className="brandBlock">
          <BrandMark compact />
          <p>Three rooms. Shared clues. One suspiciously theatrical apartment.</p>
        </div>
      ...
      <section className="centerStage">
        ...
        <div className="sceneMetaBar">
          <div>
            <span className="eyebrow">{activeRoom.kicker}</span>
        ...
        <div
          className={`roomStage ${activeRoom.palette} ${blacklightOn ? 'blacklightOn' : ''}`}
          ref={stageRef}
        ...
        >
          <RoomArt
            room={activeRoom}
            game={game}
            openHotspot={setSelectedHotspot}
            blacklightOn={blacklightOn}
            toggleBlacklight={toggleBlacklight}
          />
          {revealBurst && (
            <div className="uvRevealBurst" aria-hidden="true">
              <span />
              <span />
              <span />
              <p>Ultraviolet ink wakes up.</p>
            </div>
          )}
...
function RoomArt({ room, game, openHotspot, blacklightOn, toggleBlacklight }) {
  return (
    <div className={`tableau tableau-${room.id}`}>
      <SceneBackdrop room={room} />
      <button
        className={`secretSwitch ${blacklightOn ? 'on' : ''}`}
        onClick={toggleBlacklight}
        aria-label="Loose wall switch"
      >
        <span className="switchPlate"><span className="switchToggle" /></span>
        <span className="switchLabel">{blacklightOn ? 'The wall hums' : 'Loose wall switch'}</span>
      </button>
      <div className="uvClueLayer" aria-hidden="true">
        <span className="uvGlyph glyphOne">✶</span>
        <span className="uvGlyph glyphTwo">◐</span>
        <span className="uvGlyph glyphThree">⌁</span>
        <span className="uvGlyph glyphFour">B</span>
      </div>

      {room.hotspots.map((hotspot) => {
        const solved = hotspot.puzzle && game.solved?.[hotspot.puzzle];
        const collected = hotspot.grants?.every((id) => game.inventory?.[id]);
        const hiddenByMode = hotspot.hiddenUntilBlacklight && !blacklightOn;
        return (
          <button
            key={hotspot.id}
            className={`hotspot asset-${hotspot.shape} ${hotspot.hiddenUntilBlacklight ? 'hiddenObject' : ''} ${hiddenByMode ? 'hiddenLocked' : ''} ${solved ? 'solved' : ''} ${collected ? 'collected' : ''}`}
            style={{ left: `${hotspot.x}%`, top: `${hotspot.y}%`, width: `${hotspot.w}%`, height: `${hotspot.h}%` }}
            onClick={() => !hiddenByMode && openHotspot(hotspot)}
            disabled={hiddenByMode}
            aria-label={hiddenByMode ? `${hotspot.label} hidden until blacklight` : hotspot.label}
          >
...
function HotspotModal({ hotspot, game, close, collectItem, solvePuzzle, blacklightOn }) {
  const puzzle = hotspot.puzzle ? puzzles[hotspot.puzzle] : null;
  const [answer, setAnswer] = useState('');
  const [error, setError] = useState('');
  const [hintIndex, setHintIndex] = useState(-1);
  const solved = puzzle && game.solved?.[puzzle.id];
  const displayBody = blacklightOn && hotspot.blacklightBody ? hotspot.blacklightBody : hotspot.body;
  const puzzlePrompt = blacklightOn && puzzle?.blacklightPrompt ? puzzle.blacklightPrompt : puzzle?.prompt;
  ...
  return (
    <div className="modalScrim" onMouseDown={close}>
      <section className="modalCard" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <h2>{hotspot.title}</h2>
        <p>{displayBody}</p>
        {hotspot.blacklightBody && !blacklightOn && (
          <div className="uvNotice">This object may reveal more under blacklight.</div>
        )}
